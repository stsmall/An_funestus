...
def processSimulation(samples, positions, stars):
    newPositions = []
    ...
    if positions:
        ...
        print "\n//%s\nsegsites: %s\n%s\n%s" %(stars, len(newPositions), newPositionsStr, newSamplesStr)
    else:
        print "\n//%s\nsegsites: 0\n" %(stars)
...
while line:
    ...
    if not positionsLine.startswith("positions:"):
        ...
        if len(samples) != numSamples:
            raise Exception
        processSimulation(samples,positions, stars)
    processedSims += 1
    line = msStream.readline()
