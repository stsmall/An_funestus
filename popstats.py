...
parser.add_option("--popSFSkeinan", action="store_true", dest="popSFSkeinan",help="output pop1 SFS on one line (from 0 count to n) with correction for ascertaining in a single individual heterozygote as in Keinan et al. 2007",default=False)
...
for line in data:
	...
	if options.informative and options.f3vanilla:
		alleles1=list(set(genotypes1+genotypes2))
...
if options.SFS or options.SFS2:
	...
	for i in range(0,targetpop2count):
		...
		print i,'\t',t_list.count(i),'\t',n_list.count(i),'\t',freq #
	exit(0)
	...
	if options.popSFSkeinan:
		#print targetpop1count,SFSdict
		total=1.0*sum([SFSdict[pdic] for pdic in range(0,targetpop1count+1)])
		toprintdict=[]
		for pdic in range(1,targetpop1count):
			numerator=(SFSdict[pdic]/float(total) )
			denominator= 2.0*(1.0*pdic/targetpop1count)* (1.0-(float(pdic)/targetpop1count))
			#print pdic,numerator,denominator
			corrfreq=numerator/denominator
			toprintdict.append(str(corrfreq))
	else:
